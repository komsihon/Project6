...
from tsunami.models import Bundle, ComCampaign, Photo, PromoCode, BundleType, CloudCashIn

logger = logging.getLogger('ikwen')


class Home(TemplateView):
    template_name = 'tsunami/home.html'
...
class CCI(TemplateView):
    template_name = 'tsunami/cloud_cash_in.html'
...
#
# @login_required
# def getMemberCumulatedCoupon(request):
...
#     cumulated_coupon = []
#     for coupon in cumul:
#         user_coupon = {'count': coupon.count, 'coupon': coupon.coupon.to_dict()}
#         cumulated_coupon.append(user_coupon)
#     return HttpResponse(json.dumps(
#         {'success': True,
#          'cumulated_coupons': cumulated_coupon,
#          }), 'content-type: text/json')
...
def get_member_cumulated_coupon(request, *args, **kwargs):
    ...
    service = get_service_instance()
    coupon_list = list(Coupon.objects.using('umbrella').filter(service=service))
    coupons = CumulatedCoupon.objects.using('umbrella').filter(member=member, coupon__in=coupon_list)
    context = {'coupons': coupons,}
    return render(request, 'tsunami/snippets/user_coupon_list.html', context)
